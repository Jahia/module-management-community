...
public class ModuleManagementMutationResult {
    private transient Logger logger = LoggerFactory.getLogger(ModuleManagementMutationResult.class);

    @Inject
    @GraphQLOsgiService(
            service = ProvisioningManager.class
    )
    ProvisioningManager provisioningManager;

    @GraphQLField
    @GraphQLName("updateModules")
    @GraphQLDescription("Return a list of modules that have updates available if dryRun is true or the list of modules that have been updated")
    public List<String> updateModules(@GraphQLName("jahiaOnly") @GraphQLDefaultValue(GqlUtils.SupplierTrue.class) boolean jahiaOnly,
                                      @GraphQLName("dryRun") @GraphQLDefaultValue(GqlUtils.SupplierTrue.class) boolean dryRun,
